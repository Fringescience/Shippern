...
<?php include 'header.php';?>
        ...
        <section>
         <!-- Gives an overview of the cargo on a selected ship -->
         <form method="post">
         Select ship: <input type="number" name="ShipID">
         <input type="submit">
         </form>
         <?php 
           $query = "SELECT Ship.ShipID , Orders.OrderNr FROM Orders WHERE Orders.ShipID=$_POST[ShipID]";
           $result = mysql_query($query);
           
           echo "<table>";
           
           while($row = mysql_fetch_array($result)){   //Creates a loop to loop through results
           	echo "<tr><td>" . $row['OrderNr'] . "</td><td>" . $row['CustomerNr'] . "</td><td>" . $row['DestinationID'] . "</td><td>" . $row['Weight'] . "</td>
		          <td>" . $row['OrderDate'] . "</td><td>" . $row['SentDate'] . "</td><td>" . $row['PaidDate'] . "</td><td>" . $row['Category'] . "</td></tr>";
           }
            echo "</table>";
           ?>
        </section>
