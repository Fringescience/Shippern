...
<?php include 'header.php';?>
        ...
        <section>
         <!-- Gives an overview of the route for a selected ship -->
         <form method="post">
         Select ship: <input type="number" name="ShipID">
         <input type="submit">
         </form>
         <?php 
           $query = "SELECT Ship.ShipID , Schedule.ScheduleID FROM Schedule WHERE Schedule.RouteID=Ship.RouteID TO ShipID=$_POST[ShipID]";
           $result = mysql_query($query);
           
           echo "<table>";
           
           while($row = mysql_fetch_array($result)){   //Creates a loop to loop through results
           	echo "<tr><td>" . $row['ScheduleID'] . "</td><td>" . $row['RouteID'] . "</td><td>" . $row['DestinationID'] . "</td><td>" . $row['OriginID'] . "</td></tr>";
           }
            echo "</table>";
           ?>
        </section>
